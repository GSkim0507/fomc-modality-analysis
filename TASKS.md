...
> 각 Phase 완료 시 로컬 커밋(푸시 없음, 2026-09-04부터). 상세 진행 기록은 `HANDOVER.md` 참조.
...
- [x] 10.6 조동사 추출 v3(층위 라벨 전달) + 200문장 2인 코딩 정확도
- [x] 10.7 무결성 QA 스크립트(회의일 정합, 중복·누락, 인코딩 잔재 0, v2 대비 차이 리포트) → `results/qa/QA_report.md`
- [x] 10.8 데이터 카드 `docs/09_data_card_v3.md`
...
- [x] 11.2 X-A 구문 인벤토리(층위별)
- [x] 11.3 X-B 구문 계단(Figure 4 구문판: 성명서 회의별 구문 점유율 스택 + PELT + 원인 문장 표) — 메인
- [x] 11.4 X-C 편집 이벤트·보유율 반감기(지표 1개)
- [x] 11.5 X-D 장르×귀속 분업 + will/would vs can/could 화용 대조표
- [x] 11.6 X-E 구문×거시(사전 지정 구문 8–10개, CFNAI+VIX, T1/T2 병기, HAC) — 메인
- [x] 11.7 X-F will/would 비대칭(be appropriate × 조동사 × 귀속 층 × VIX)
- [x] 11.8 X-G 선행성(CCF·Granger·증분예측) 부록화
- [x] 11.9 주 비교 3종(S1·S2·S4) × U2 × (T1,T2) 산출 확인

- [x] 11.0 CFNAI·VIX 선택 근거 문서 `docs/10_macro_indicator_rationale.md`

## Phase 12 — 비교 보고서·웹 열람 (`docs/08` §3.4)
- [x] 12.1 `results/report/` 시나리오별 페이지(표·그림·한 줄 결론·시나리오 간 차이)
- [x] 12.2 시나리오 비교 대시보드 + 한 문장 주장 후보 3개(C1–C3) 근거 격자
- [x] 12.3 정적 HTML 배포(GitHub Pages 또는 Artifact) → 팀 회의
- [x] 12.5 결과 요약 `docs/11_scenario_results_v3.md`
- [x] 12.7 실험 보고서 `paper/report_v3.md` (+ .docx, .html, 아티팩트) — 교수님 요구 '보고서 형식'
- [ ] 12.6 사람 검증 표본 채우기(화자 역할·의사록 층위·추출 200문장 2인 코딩)
...
## Phase 16 — 세미나형 결과 보고서 (가독성 재편)
- [x] 16.1 표 셀 이스케이프 버그 수정(`24_build_report_v3.py`): 셀이 `<`로 시작할 때만 원문 삽입하던 가드를 태그 탐지로 교체. 회귀표 5,230곳이 태그 문자열로 표시되던 문제
- [x] 16.2 `experiments/28_build_seminar.py`: 절별 시나리오 탭(문서 전체 동기화) 지시자 `{{cmp:tbl:…}}` `{{cmp:fig:…}}` `{{scnbar}}`
- [x] 16.3 원고 `paper/report_v3_seminar.md`: 결론 우선 구성 — 한 장 요약(답 한 문장) → 답 세 걸음 → 아닌 것 → 조합 강건성 → 결정 요청 → 부록
- [x] 16.4 §8.3에 묻혀 있던 결론 문장 후보 C1–C3를 §6 '결정해 주십시오'로 올리고 선택 기준·위험 표 추가 (docs/08 §3.4 요구사항)
- [x] 16.5 코드(C11_U2, S4, L1–L9 등) 우리말 풀어쓰기, 방법론·전수 스크린·ledger 부록 이동
- [x] 16.6 팀 야마 반영: 보고서를 세 주장(불확실성 · 장르 · 연구방법) 구조로 재작성. §0 세 주장 요약표 → §3 주장 1 → §4 주장 2 → §5 주장 3 → §6 남은 결정
- [x] 16.7 주장 1 데이터 대조: 선행성(텍스트→VIX Granger q<.10)은 72 run 중 0, 예측회귀 0/72 → '동행하는 상태변수, 선행지표 아님'으로 재정의하고 §3.3에 검정 결과를 그대로 보고. v2의 기자회견 선행 신호는 기자 발화 오염(could 48%·might 50%)에서 나온 것임을 명시
- [x] 16.8 조동사 단위 vs 구문 단위 대조표(세 조합): 확정 VIX 0/0/5 → 1/3/8, 인공물 2/3/12 → 7/12/35, 계단 14(10) → 25(17). 구문 단위와 확정 규칙이 한 쌍임을 §3.2·§5.2에 기술
- [x] 16.9 보고서 본체 CSS에 `word-break:keep-all` — 제목이 단어 중간에서 갈라지던 문제
- [ ] 16.10 팀 검토 → 의사록 포함(D8)·주 조합(D10) 확정, 사람 검증 표본 채우기
